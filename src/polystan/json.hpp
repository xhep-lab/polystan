...
class Object {
 public:
  ...
  void add(std::string name, std::vector<T> data) {
    value.AddMember(String(name, alloc).Move(), Vector(data, alloc).Move(), alloc);
  }
  ...
  void add(std::string name, const char* data) {
    value.AddMember(String(name, alloc).Move(), String(data, alloc).Move(), alloc);
  }
